...
public class GreenButton extends FloorEntity implements Actable {

	private boolean isActive;
	
	public GreenButton(int posRow, int posCol) {
	...
	public void changeSprite() {
		if (isActive) {
			setCurrentSprite(new Image(ClassLoader.getSystemResource("img/entities/greenbuttonactivate.png").toString()));
		}
		else {
			setCurrentSprite(new Image(ClassLoader.getSystemResource("img/entities/greenbutton.png").toString()));
		}
	...
	public void update() {
		...
		if ((((GameLogic.getPlayer().getPosRow() == getPosRow()) && (GameLogic.getPlayer().getPosCol() == getPosCol())) || (upperEntity instanceof SolidEntity)) && (!isActive)) {
			...
			for (int i = 0; i < 10; i++) {
				for (int j = 0; j < 10; j++) {
					if (tiles[i][j] instanceof GreenPlatform) {
						tiles[i][j] = ((GreenPlatform) tiles[i][j]).getChangedTile();
						if (GameLogic.getMap().getEntities()[i][j] instanceof Destroyable) {
							((Destroyable) GameLogic.getMap().getEntities()[i][j]).destroy();
						}
					}
					else if (tiles[i][j] instanceof GreenEmptyPlatform) {
						tiles[i][j] = ((GreenEmptyPlatform) tiles[i][j]).getChangedTile();
					}
				}
			}
		}
